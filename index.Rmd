...
res <- getRates(grp_data) %>% 
  left_join(
    grp_data %>% 
      mutate(addWx_dayn = as.Date(add_wx_time) - as.Date(user_class_start_time)) %>%
      summarise(
        .enroll_total = n_distinct(user_id, na.rm = TRUE),
        add_wx_d1_rate = n_distinct(ifelse(addWx_dayn <= 0, 
                                           user_id, NA), na.rm = TRUE)/.enroll_total,
        add_wx_d2_rate = n_distinct(ifelse(addWx_dayn <= 1, 
                                           user_id, NA), na.rm = TRUE)/.enroll_total,
        add_wx_d3_rate = n_distinct(ifelse(addWx_dayn <= 2, 
                                           user_id, NA), na.rm = TRUE)/.enroll_total,
        .groups = "drop_last"
      ) %>% 
      mutate(across(ends_with("rate"), ~ round(.x *100, 0))) %>% 
      select(-.enroll_total)
    , by = group_vars(grp_data)
  ) %>% 
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = apply_rate)) + 
    ...
    geom_text(aes(label = apply_rate)) + 
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = add_wx_rate)) + 
    ...
    geom_line(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_line(aes(y = add_wx_d1_rate, group = group_class, color = group_class),
              color = "grey") + 
    geom_line(aes(y = add_wx_d2_rate, group = group_class, color = group_class),
            color = "grey") + 
    geom_line(aes(y = add_wx_d3_rate, group = group_class, color = group_class),
            color = "grey") + 
    geom_text(aes(label = add_wx_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
```{r}
gg <- res %>% 
  filter(term_name %in% c("2021暑假-第1期","2021暑假-第2期","2021暑假-第3期","2021暑假-第4期")) %>% 
  ggplot(aes(x = group_class, y = add_wx_rate)) + 
    geom_point(aes(color = term_name)) +
    geom_line(aes(group = term_name, color = term_name, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_line(aes(y = add_wx_d1_rate, group = term_name, color = term_name),
              color = "grey") + 
    geom_line(aes(y = add_wx_d2_rate, group = term_name, color = term_name),
            color = "grey") + 
    geom_line(aes(y = add_wx_d3_rate, group = term_name, color = term_name),
            color = "grey") + 
    geom_text(aes(label = add_wx_rate)) + 
    facet_grid(. ~ term_name, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = unit1_attend_rate)) + 
    ...
    geom_line(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_text(aes(label = unit1_attend_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = unit5_finish_rate)) + 
    ...
    geom_line(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_text(aes(label = unit5_finish_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = unit5_finish_rate)) + 
    geom_point(aes(color = group_class, text = group_name), size = 0.8) +
    geom_boxplot(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    #geom_text(aes(label = unit5_finish_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = retent_rate)) + 
    ...
    geom_line(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_text(aes(label = retent_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
gg <- res %>% 
  ggplot(aes(x = term_name, y = unit5_apply_rate)) + 
    ...
    geom_line(aes(group = group_class, color = group_class, 
                   text = paste0("招生量:",enroll_rate))) + 
    geom_text(aes(label = unit5_apply_rate)) + 
    facet_grid(. ~ group_class, scales = "free") +
    theme(legend.position = "none")
ggplotly(gg)
```
...
highchart() %>% 
  hc_add_series(res, "line", hcaes(x = term_label, y = enroll_rate, group = l1_order_channel)) %>% 
  hc_xAxis(title = "", type = "category", categories = unique(res$term_label),
          plotBands = list(list(
              label = list(text = ""),
              color = "rgba(100, 0, 0, 0.1)",
  ...
  hc_colors(c("black","#53b400","#00b6eb","lightgrey","#c9a215","#a58aff","#ee6bda","lightgrey")) %>%
  hc_legend(verticalAlign = "top") %>% 
  hc_plotOptions(
        series = list(
          dataLabels = list(enabled = TRUE)
...
```{r}
res <- res %>% filter(enroll_rate > 3, apply_rate > 2, !is.na(l1_order_channel))

highchart() %>% 
  hc_add_series(res, "line", 
                hcaes(x = term_label, y = apply_rate, group = l1_order_channel)) %>% 
  hc_add_series(res, type = "scatter", 
                hcaes(x = term_label, y = apply_rate, group = l1_order_channel, size = enroll_rate),
                maxSize = "10%", dataLabels = list(enabled = FALSE)) %>%
  hc_colors(c("#53b400","#00b6eb","lightgrey","#c9a215","#a58aff","#ee6bda")) %>%
  hc_xAxis(title = "", type = "category", categories = unique(res$term_label),
          plotBands = list(list(
              label = list(text = ""),
              color = "rgba(100, 0, 0, 0.1)",
...
highchart() %>% 
  ...
  hc_add_series(res, type = "scatter", 
                hcaes(x = term_label, y = apply_rate, group = pay_grade, size = enroll_rate),
...
```{r}
by_date_pay_join <- dat %>% 
  filter(
    l1_pay_time > "2021-01-01") %>% 
  mutate(
    l1_pay_time = as.Date(l1_pay_time),
    user_class_start_time = as.Date(user_class_start_time)) %>% 
  group_by(term_label, l1_pay_time, user_class_start_time) %>% 
  summarise(
    enroll_total = n_distinct(user_id, na.rm = TRUE),
    ct_total = n_distinct(counselor_id, na.rm = TRUE),
    add_wx_rate = mean(ifelse(is.na(add_wx_time), 0, 1)),
  ...
  arrange(desc(term_label), desc(l1_pay_time), desc(user_class_start_time))
```

```{r}
names_dt <- names(by_date_pay_join)
DT::datatable(
  by_date_pay_join,
    class = "display compact",
    #filter = "top",
